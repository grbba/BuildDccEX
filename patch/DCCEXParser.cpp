...
const int HASH_KEYWORD_LATCH = 1618;  // ON: Latching diag output to the channel on which the command was recieved; OFF back to Serial;
...
void DCCEXParser::parse(Print *stream, byte *com, RingStream * ringStream)
{
    ...
    case '+': // Complex Wifi interface command (not usual parse)
        DIAG(F("Parsing + command")); //  %s"), com);
        if (atCommandCallback) {
          DIAG(F("Parsing + command and sending to atcallback")); // : %s"), com);
          if (DCCWaveform::isRunning()) {
...
bool DCCEXParser::parseD(Print *stream, int params, int p[])
{
    ...
    {
//grbba
    case HASH_KEYWORD_LATCH: // <D LATCH ON/OFF>
        Diag::LATCH = onOff;
        Latch::type = Connection::type; // Connection is set at each incomming command
        Latch::id = Connection::id;     // same as the id
        return true;
//grbba
